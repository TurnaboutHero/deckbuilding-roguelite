...
반복 가능한 플립 스킬은 손패와 합법적인 대상이 남아 있는 동안 같은 턴에 여러 번 즉시 사용할 수 있다. 각 사용은 동전과 대상을 선택한 뒤 플립과 효과를 끝까지 해결하고, 결과를 확인한 다음 다음 행동을 선택한다. 쿨타임·턴당 1회·전투당 1회·일회성·명시적 반복 불가가 적용된 스킬은 사용 직후 잠긴다. 행동 예약과 실행 큐는 만들지 않는다.
